...
void LAN::Init(void) {
  ...
  Thread = osThreadNew([](void *argument) {Lan.Task();}, NULL, &attr);
	if(Thread == 0) {
		while(true);
	}
}
		...
		for(U8 Index_Port=0; Index_Port<Lan.NumberOfListen; Index_Port++) {
			if(Media_Listen[NumberOfListen].Ring.Socket) {			
				if(Media_Listen[Index_Port].Ring.Send.Length != Media_Listen[Index_Port].Ring.Send.Sent) {
					...
					if (netTCP_SendReady (Media_Listen[NumberOfListen].Ring.Socket)) {
						...
						if(sendbuf) {
							...
							netTCP_Send (Media_Listen[NumberOfListen].Ring.Socket, sendbuf, Media_Listen[Index_Port].Ring.Send.Length);
